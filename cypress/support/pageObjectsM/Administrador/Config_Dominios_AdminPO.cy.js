class Config_Dominios_AdminPO {
  ...
  btnCancelar(t) {
    ...
    cy.visit(
      "https://laureateqa.axity.com/sdig/uvm/admin/settings/domains/admin"
    );
    cy.wait(tiempo);
    cy.get(".route").contains(
      "Configuraciones / Dominios / Portal administración"
    );
    cy.wait(tiempo);
    ...
    cy.wait(tiempo);
    cy.screenshot(' Dominios administración - Btn cancelar', { capture: "fullPage" });
  }
  ...
  camposVacios(t) {
    ...
    cy.get(".btn-primary > .mdc-button__label").click();
    cy.wait(tiempo);
    cy.screenshot(' Dominios administración - Campos vacios', { capture: "fullPage" });
  ...
  happyPath(t) {
    ...
    for (let num = 1; num <= numero_test; num++) {
      ...
      cy.wait(tiempo);
      cy.screenshot(' Dominios administración - HappyPath', { capture: "fullPage" });
  ...
  otros(t) {
    ...
    cy.wait(tiempo);
    cy.screenshot(' Dominios administración - Buscador', { capture: "fullPage" });
    ...
    cy.wait(tiempo);
    cy.screenshot(' Dominios administración - Exportar', { capture: "fullPage" });
  }
} //final

export default Config_Dominios_AdminPO;
